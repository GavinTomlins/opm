...
| `opm preset <verb>` | Model presets: `list`, `show`, `use`, `diff`, `capture`, `create`, `models`, `edit`, `status`, `revert` — named model-mapping overlays applied surgically onto the active profile's oh-my-openagent config (the one deliberate exception to the flat-surface rule; see `docs/superpowers/specs/2026-07-13-model-presets-design.md`) |
